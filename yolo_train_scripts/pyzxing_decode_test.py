...
def pyzxing_decode(image):
    ...
    try:
        ...
        # pyzxing需要文件路径，先保存临时文件
        temp_path = f"temp_decode_{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}.png"
        cv2.imwrite(temp_path, image)
        results = decoder.decode(temp_path)
    ...
    try:
        ...
        if yolo_results[0].boxes is not None:
            ...
            for i, box in enumerate(yolo_results[0].boxes):
                ...
                if region.size > 0:
                    print(f"    🔍 使用pyzxing解码:")
                    decoded_results = pyzxing_decode(region)
        ...
        print(f"\n🔍 对比测试: 全图直接pyzxing解码")
        print("-" * 30)
        full_image_results = pyzxing_decode(image)
